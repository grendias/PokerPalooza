...
class Repository
{
	...
	{
		if (!$player['IsRebuy']) $player['IsRebuy'] = 0;
		if (!$player['IsBoost']) $player['IsBoost'] = 0;
		try
		{
	...
	function UpsertLog($level, $source, $message)
	{
		try
		{
			$sql = '
			...
			{
				echo("Something went wrong while trying to send logs: {$result['message']}");
			}
		}
		catch (Exception $e)
		{
